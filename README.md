...
[![Language][cmake-badge]](https://cmake.org)
[![Version][cmake-version-badge]][cmake-version]
...
Where `util-linux-tag` is the release tag in [util-linux][util-linux] such as `v2.39`, `v2.39.1`, etc.
...
since on Linux you might as well just use the original util-linux build).
Note that man pages will only be generated if you have [Asciidoctor][asciidoctor] available.

1. Download or clone this repository into SOME_PATH
2. Do something like this on the command line
```bash
cd SOME_PATH
cmake -S . -B build 
cmake --build build
...
* `uuid::uuid` - the "default" one. If only one of the static/shared variants is enabled, this one points to it. 
...
[util-linux-license]: https://github.com/util-linux/util-linux/blob/master/libuuid/COPYING
...
[tests-badge]: https://github.com/gershnik/libuuid-cmake/actions/workflows/test.yml/badge.svg
[tests]: https://github.com/gershnik/libuuid-cmake/actions/workflows/test.yml
